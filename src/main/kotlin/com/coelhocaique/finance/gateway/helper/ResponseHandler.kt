...
import com.coelhocaique.finance.gateway.helper.Messages.DEFAULT_ERROR_MESSAGE
...
object ResponseHandler {
    ...
    private fun mapApiException(it: ApiException): Mono<ServerResponse> {
        logger().error("status=".plus(it.status)
                .plus(", cause=")
    ...
    private fun buildErrorResponse(message: String?) = ErrorResponse(listOf(message
